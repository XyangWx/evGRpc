...
class TestErrorPath:
    ...
    def test_list_consumptions_int_max_page_size_caps_to_1000(
        self, channel, namespace
    ):
        """page_size=INT_MAX → capped (Bug #5 fix)."""
        stub = rpc.ConsumptionServiceStub(channel)
        resp = stub.ListConsumptions(pb.ListConsumptionsRequest(
            page_size=2147483647
        ))
        assert isinstance(resp, pb.ListConsumptionsResponse)
